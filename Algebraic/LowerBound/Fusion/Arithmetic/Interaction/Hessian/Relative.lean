...
theorem residualRank_le_twice_with_helpers
    (constant : C → K) (target : MvPolynomial (Fin n) K)
    (given : Fin k → MvPolynomial (Fin n) K) (point : Fin n → K) :
    residualRank point target given ≤ 2 * Circuit.relativeCostComplexity
      (Algebraic.Arithmetic.interpretation (fun scalar => MvPolynomial.C (constant scalar)))
      (Algebraic.Arithmetic.multiplicationCost (K := C))
      (fun (_ : Unit) (_ : Fin 1) => target)
      (fun _ => Fin.append MvPolynomial.X given) := by
  have discard_variables : residualRank point target given ≤
      residualRank point target (Fin.append MvPolynomial.X given) := by
    unfold residualRank
    refine le_iInf fun coefficients => ?_
    have sum_eq : ∑ i, coefficients i • linearMap point (Fin.append MvPolynomial.X given i) =
        ∑ j, coefficients (Fin.natAdd n j) • linearMap point (given j) := by
