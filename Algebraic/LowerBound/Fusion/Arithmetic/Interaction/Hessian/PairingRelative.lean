...
theorem rank_hessian_residual (point : (Fin n ⊕ Fin n) → K)
    (given : Fin k → MvPolynomial (Fin n) K) (coefficients : Fin k → K) :
    LinearMap.rank (linearMap point (polynomial K n) -
      ∑ j, coefficients j • linearMap point (MvPolynomial.rename Sum.inl (given j))) =
        2 * n := by
  let correction := ∑ j, coefficients j •
    linearMap point (MvPolynomial.rename Sum.inl (given j))
...
def preprocessedSources (given : Fin k → MvPolynomial (Fin n) K) :
    Fin (2 * n + k) → MvPolynomial (Fin n ⊕ Fin n) K :=
  Fin.append (fun i => MvPolynomial.X (variableEquiv n i))
    (fun j => MvPolynomial.rename Sum.inl (given j))

/-- Exact formal multiplication complexity with left-only preprocessing. -/
def preprocessedMultiplicationComplexity (given : Fin k → MvPolynomial (Fin n) K) : ℕ∞ :=
  Circuit.relativeCostComplexity
    ...
    (fun _ => preprocessedSources given)
...
theorem preprocessedMultiplicationComplexity_lowerBound
    (given : Fin k → MvPolynomial (Fin n) K) :
    (n : ℕ∞) ≤ preprocessedMultiplicationComplexity given := by
  apply Hessian.relativeCostComplexity_lowerBound (fun x : K => x)
    (polynomial K n) (preprocessedSources given) (0 : (Fin n ⊕ Fin n) → K) n
  intro coefficients
...
theorem preprocessedMultiplicationComplexity_upperBound
    (given : Fin k → MvPolynomial (Fin n) K) :
    preprocessedMultiplicationComplexity given ≤ n := by
  ...
  have computes : expression.circuit.ComputesFrom
      ...
      (fun (_ : Unit) (_ : Fin 1) => polynomial K n)
      (fun _ => preprocessedSources given) := by
    intro x
    funext output
...
theorem preprocessedMultiplicationComplexity_eq
    (given : Fin k → MvPolynomial (Fin n) K) :
    preprocessedMultiplicationComplexity given = n :=
  le_antisymm (preprocessedMultiplicationComplexity_upperBound given)
    (preprocessedMultiplicationComplexity_lowerBound given)
