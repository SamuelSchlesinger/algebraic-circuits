...
private def withCoordinates (given : Fin k → Fin n → ZMod 2) :
    Fin (n + k) → Fin n → ZMod 2 :=
  Fin.append (fun i => Pi.single i 1) given

private theorem forms_withCoordinates (given : Fin k → Fin n → ZMod 2) :
    forms (withCoordinates given) = fun input => Fin.append input (forms given input) := by
  funext input i
  refine Fin.addCases (fun i => ?_) (fun j => ?_) i
  · simp [forms, withCoordinates]
  · simp [forms, withCoordinates]

private theorem representation_split (given : Fin k → Fin n → ZMod 2)
    (coefficients : Fin (n + k) → ZMod 2) :
    ∑ i, coefficients i • withCoordinates given i =
      (fun i => coefficients (Fin.castAdd k i)) +
        ∑ j, coefficients (Fin.natAdd n j) • given j := by
  rw [Fin.sum_univ_add]
...
theorem conditionalGateComplexity_eq_min_weight
    ...
    (addition_eval : ∀ input, addition.eval interpretation input 0 = input 0 + input 1)
    (target : Fin n → ZMod 2) (nonzero : target ≠ 0)
    (given : Fin k → Fin n → ZMod 2) :
    Circuit.conditionalGateComplexity interpretation
      (fun input (_ : Fin 1) => target ⬝ᵥ input) (forms given) =
      ⨅ coefficients : Fin k → ZMod 2,
        ((weight coefficients + weight (target + ∑ j, coefficients j • given j) - 1 : Nat) : ℕ∞) := by
  change Circuit.relativeGateComplexity interpretation _
    (fun input => Fin.append input (forms given input)) = _
  ...
  · refine le_iInf fun coefficients => ?_
    let combined := Fin.append (target + ∑ j, coefficients j • given j) coefficients
    have represents : ∑ i, combined i • withCoordinates given i = target := by
