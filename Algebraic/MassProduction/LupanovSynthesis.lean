...
This module builds on the pattern banks in `LupanovPatternBank` to formalize
the block-table construction behind the sharp one-copy upper bound. It
recombines matching address and data flags, proves exact semantics, and selects
uniform parameters for the coefficient-one estimate.
