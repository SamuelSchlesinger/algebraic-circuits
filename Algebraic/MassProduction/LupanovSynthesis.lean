...
This module builds on the truth-table representation in `LupanovTable` to
formalize the block-table construction behind the sharp one-copy upper bound.
For each block and Boolean pattern, paired circuits recognize the indicated
address rows and data columns; their conjunctions reconstruct the target truth
table.
