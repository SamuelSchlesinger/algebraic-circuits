...
import Cslib.Computability.Circuit.Boolean.Basic
...
private def fromBooleanOperation : (op : Boolean.Op) →
...
private def toBooleanGateCount : Op → Nat
  | .id => 0
  | .false | .true | .not | .and | .or => 1

private def toBooleanOperation : (op : Op) →
...
theorem boolean_computes_iff (circuit : Circuit Boolean.signature n g 1)
    (function : Boolean.BooleanFunction n) :
    circuit.Computes function ↔
...
theorem toBoolean_computes (circuit : Circuit signature n g 1)
    ...
    (toBoolean.compile circuit).Computes function ↔
